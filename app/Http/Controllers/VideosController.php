...
class VideosController extends Controller
{
    ...
    public function getVideo(Request $request)
    {
        ...
        //first check student can show branch admin videos or not 
        ...
        // if($show_video->studentDetails->show_admin_videos == "Y"){
            ...
            if($branch_show_video->institue->show_admin_videos == "Y"){
                if($design_id !='' && $section_id != ''){
                    ...
                    $data = Videos::latest()->where("user_id",$branch_admin_id)->orWhere("user_id",$superadmin_id)->where("section_id",$section_id)->paginate(10);
                }else{
                    $data = Videos::latest()->where("user_id",$branch_admin_id)->orWhere("user_id",$superadmin_id)->paginate(10);
                }
            }else{
                if($design_id !='' && $section_id != ''){
                    $data = Videos::latest()->where("user_id",$branch_admin_id)->where("section_id",$section_id)->where("design_id",$design_id)->paginate(10);
                }else if($section_id != ''){
